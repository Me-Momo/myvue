...
<audio id='player' :src='audio.mp3Url' autoplay @timeupdate='change' @ended='next' @error='next' @dataunavaliable='next'></audio>
</div>
</div>
...
export default {
  ...
  computed: {
    ...mapGetters(['audio', 'isPlay', 'loadingState', 'currentIndex','togglePlaylist','audioList'])
  },
  methods: {
    init: function() {
      Indicator.open({
      ...
      var that=this;
      setTimeout(()=>{
        that.$store.commit('playAudio', that.currentIndex);
        that.$store.commit('isPlay', true);
      },2000);
    }, //init end
    play() {
      this.$store.commit('playAudio', this.currentIndex);
    ...
    prev() {
      if (this.currentIndex !== 0) {
        this.$store.commit('playAudio', this.currentIndex - 1);
      }
    },
    next() {
